...
/// update next states as well as value of the output 
    always@(*)   
    begin
    case(state)
      idle :
       begin
               tx_done     = 1'b0; 
               tx          = 1'b1;    // this is 1 as we have start bit as zero 
               tx_reg      = {(8){1'b0}}; 
               tx_err      = 0;
               if(tx_start) 
      ...
      send_data:
       begin
                if(count < (length - 1))  // as we will send the last count in else if and else stage 
                   begin
                     next_state = send_data;
                     tx         = tx_reg[count];
                   end
